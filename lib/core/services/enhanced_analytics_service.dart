...
class EnhancedAnalyticsService {
  ...
  void initialize() {
    ...
    if (kDebugMode) {
      debugPrint('EnhancedAnalyticsService initialized');
    }
  }
  ...
  // Portfolio Analytics
  void trackPortfolioPerformance(
      String userId, Map<String, dynamic> portfolioData) {
    ...
    final performance = totalValue > 0
  ...
  void trackTransaction(String userId, Map<String, dynamic> transaction) {
    ...
    _transactionHistory[key] =
        _transactionHistory[key] ?? <Map<String, dynamic>>[];
  ...
  void trackCommunityActivity(
      String communityId, String activityType, Map<String, dynamic> data) {
  ...
  void trackChatbotInteraction(
      String interactionType, Map<String, dynamic> context) {
    _chatbotInteractions[interactionType] =
        (_chatbotInteractions[interactionType] ?? 0) + 1;
  ...
  Map<String, dynamic> getUserInsights(String userId) {
    ...
    return {
      ...
      'lastActive':
          sessions.isNotEmpty ? sessions.last.toIso8601String() : null,
  ...
  Map<String, dynamic> _calculatePortfolioAnalytics(
      List<Map<String, dynamic>> transactions) {
    if (transactions.isEmpty) return {};

    final buyTransactions =
        transactions.where((t) => t['type'] == 'buy').length;
    final sellTransactions =
        transactions.where((t) => t['type'] == 'sell').length;
    ...
    for (final entry in _volumeAnalysis.entries) {
      ...
      if (volumes.isNotEmpty) {
        ...
        analytics[entry.key] = {
          ...
          'trend': volumes.length > 1
              ? (volumes.last - volumes[volumes.length - 2])
              : 0.0,
        };
      }
    }
  ...
  Map<String, dynamic> _calculateEngagementTrends() {
    ...
    return {
      ...
      'averageEngagementPerPost': _postEngagement.isNotEmpty
          ? _postEngagement.values.fold(0, (a, b) => a + b) /
              _postEngagement.length
          : 0.0,
  ...
  List<Map<String, dynamic>> _getTopScreens() {
    final screens = _screenViews.entries
        .map((e) => {
              'screen': e.key,
              'views': e.value,
            })
  ...
  List<Map<String, dynamic>> _getTopFeatures() {
    final features = _featureUsage.entries
        .map((e) => {
              'feature': e.key,
              'usage': e.value,
            })
  ...
  List<Map<String, dynamic>> _getTopEngagementTypes() {
    ...
    final sorted = engagements.entries
        .map((e) => {
              'type': e.key,
              'count': e.value,
            })
